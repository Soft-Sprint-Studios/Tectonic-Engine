...
LoadedModel* Model_Load(const char* path) {
    ...
    if (cgltf_load_buffers(&options, data, path) != cgltf_result_success) { cgltf_free(data); return NULL; }
    LoadedModel* loadedModel = malloc(sizeof(LoadedModel));
    if (!loadedModel) { cgltf_free(data); return NULL; }
    ...
    if (!loadedModel->meshes) { free(loadedModel); cgltf_free(data); return NULL; }
    ...
    for (size_t i = 0; i < data->meshes_count; ++i) {
        ...
        for (size_t j = 0; j < mesh->primitives_count; ++j) {
            ...
            for (size_t k = 0; k < primitive->attributes_count; ++k) {
                ...
                if (attr->type == cgltf_attribute_type_position) { positions = malloc(vertexCount * 3 * sizeof(float)); cgltf_accessor_unpack_floats(attr->data, positions, vertexCount * 3); 
