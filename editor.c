...
void Editor_RenderUI(Engine* engine, Scene* scene, Renderer* renderer) {
    ...
    if (g_EditorState.selected_entity_type == ENTITY_MODEL && g_EditorState.selected_entity_index < scene->numObjects) {
        ...
        Brush* b = &scene->brushes[g_EditorState.selected_entity_index];
        if (UI_Selectable("Is Reflection Probe", b->isReflectionProbe)) {
            Undo_BeginEntityModification(scene, ENTITY_BRUSH, g_EditorState.selected_entity_index);
            b->isReflectionProbe = !b->isReflectionProbe;
        ...
        if (UI_Selectable("Is Trigger", b->isTrigger)) {
            Undo_BeginEntityModification(scene, ENTITY_BRUSH, g_EditorState.selected_entity_index);
        ...
        UI_Selectable("Paint Mode Active (0)", &g_EditorState.is_painting_mode_enabled);
        if (g_EditorState.is_painting_mode_enabled) {
    ...
    if (UI_CollapsingHeader("Fog", 1)) { if (UI_Selectable("Enabled", scene->fog.enabled)) { scene->fog.enabled = !scene->fog.enabled; } UI_ColorEdit3("Color", &scene->fog.color.x); UI_DragFloat("Start Distance", &scene->fog.start, 0.5f, 0.0f, 5000.0f); UI_DragFloat("End Distance", &scene->fog.end, 0.5f, 0.0f, 5000.0f); }
    if (UI_CollapsingHeader("Post-Processing", 1)) { if (UI_Selectable("Enabled", scene->post.enabled)) { scene->post.enabled = !scene->post.enabled; } UI_Separator(); UI_Text("CRT & Vignette"); UI_DragFloat("CRT Curvature", &scene->post.crtCurvature, 0.01f, 0.0f, 1.0f); UI_DragFloat("Vignette Strength", &scene->post.vignetteStrength, 0.01f, 0.0f, 2.0f); UI_DragFloat("Vignette Radius", &scene->post.vignetteRadius, 0.01f, 0.0f, 2.0f); UI_Separator(); UI_Text("Effects"); if (UI_Selectable("Lens Flare", scene->post.lensFlareEnabled)) { scene->post.lensFlareEnabled = !scene->post.lensFlareEnabled; } UI_DragFloat("Flare Strength", &scene->post.lensFlareStrength, 0.05f, 0.0f, 5.0f); UI_DragFloat("Scanline Strength", &scene->post.scanlineStrength, 0.01f, 0.0f, 1.0f); UI_DragFloat("Film Grain", &scene->post.grainIntensity, 0.005f, 0.0f, 0.5f); UI_Separator(); UI_Text("Depth of Field"); if (UI_Selectable("Enabled##DOF", scene->post.dofEnabled)) { scene->post.dofEnabled = !scene->post.dofEnabled; } UI_DragFloat("Focus Distance", &scene->post.dofFocusDistance, 0.005f, 0.0f, 1.0f); UI_DragFloat("Aperture", &scene->post.dofAperture, 0.5f, 0.0f, 200.0f); }
